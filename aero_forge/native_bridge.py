...
extension (via the existing transpiler and cargo) and then executed from Python
without spawning a new subprocess for every call.  Compilation is keyed by the
UAST of the decorated function, so repeated calls hit a content-addressable node
cache.
"""
...
class NativeAccelerator:
    """Compile and call a Python function through a Rust native extension."""
