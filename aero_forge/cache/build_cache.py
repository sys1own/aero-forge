...
class BuildCache:
    ...
    def __init__(self, root: Optional[Path] = None, enabled: bool = True):
        self.root = (root or Path.home() / ".cache" / "aero-forge" / "build_cache").resolve()
    ...
    def _key(self, source: str, compiler_flags: list, function_name: str) -> str:
        material = f"{source}::{','.join(compiler_flags)}::{function_name}".encode("utf-8")
        return hashlib.sha256(material).hexdigest()

    def get(self, source: str, compiler_flags: list, function_name: str) -> Optional[Path]:
        if not self.enabled:
            return None
