        ...
        for entry in list(blueprint.manifest):
            ...
            if entry.lang == "python":
                if path.name == "__init__.py":
                    ...
                    entrypoint = EntrypointAdapterEngine(
                        execution_strategy,
                        str(self.workspace),
                    ...
                    if entrypoint and Path(entrypoint).is_file():
                        ...
                        # Non-Python primary entrypoints still need a Python main
                        # module so the workspace manifest is complete and run_shell
                        # can be imported without error.
                        content = (
                            f"# {path.name} placeholder generated by aero-forge\n"
                            "def main() -> int:\n"
                            "    print('Native binary entrypoint not yet implemented')\n"
                            "    return 0\n"
                        )
                    # Fall through to the content write below.
