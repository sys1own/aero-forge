...
fn aero_forge_native(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(hin_engine::detect_hin_stall, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(deterministic_healer::align_return_type, m)?)?;
