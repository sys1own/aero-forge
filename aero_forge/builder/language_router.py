...
class SystemToolchainRouter:
    ...
    def _tokenized_cargo_flags(cls, compiler_flags: Sequence[str]) -> List[str]:
        ...
        Strips any ``--target <wasm32-triple>`` tokens because the host cargo
        build must produce the native cdylib; wasm cross-compilation is handled
        by the separate ``wasm/`` crate generated by the materializer.
        """
        import shlex

        tokens: List[str] = []
        for flag in compiler_flags or []:
            toks = shlex.split(str(flag))
            i = 0
            while i < len(toks):
                if toks[i] == "--target" and i + 1 < len(toks):
                    if "wasm" in toks[i + 1].lower():
                        i += 2
                        continue
                if toks[i].lower().startswith("wasm") or toks[i].startswith("--target=wasm"):
                    i += 1
                    continue
