...
class LogEvaluator:
    ...
    RUST_STRUCTURAL_PATTERNS: List[Tuple[str, str, str]] = [
        (
            ...
            "rust_syntax",
            "Unbalanced closing delimiter; structural AST patch can restore balance.",
        ),
        (
            r"mismatched closing delimiter",
            "rust_syntax",
            "Mismatched closing delimiter; structural AST patch can restore balance.",
        ),
        (
            r"unclosed delimiter",
            "rust_syntax",
            "Unclosed delimiter; structural AST patch can close it.",
        ),
        (
            r"expected `;`",
            "rust_syntax",
            "Missing semicolon; structural AST patch can insert it.",
        ),
        (
        ...
        (
            r"expected one of",
            "rust_syntax",
            "Syntax issue; may be repairable by structural AST patch.",
        ),
        (
            r"dangling.*(?:brace|bracket|paren)",
            "rust_syntax",
    ...
    RUST_SEMANTIC_PATTERNS: List[Tuple[str, str, str, str]] = [
        (
            ...
            "type_mismatch",
            "E0308",
            "Mismatched types. Requires type refactoring rather than AST overlay.",
        ),
        (
            r"error\[E0425\]",
            "value_not_found",
            "E0425",
            "Cannot find value in scope. Requires code refactoring or symbol introduction.",
        ),
        (
            r"error\[E0277\]",
            "trait_not_satisfied",
            "E0277",
            "Trait not satisfied. Requires trait implementation or bound changes.",
        ),
        (
            r"error\[E0432\]",
            "unresolved_import",
            "E0432",
            "Unresolved import. Requires module path or dependency fix.",
        ),
        (
            r"error\[E0433\]",
            "unresolved_import",
            "E0433",
            "Failed to resolve use path. Requires module path or dependency fix.",
        ),
        (
            r"error\[E0061\]",
            "argument_mismatch",
            "E0061",
            "Method called with wrong number of arguments. Requires signature refactor.",
        ),
        (
            r"error\[E0599\]",
            "method_not_found",
            "E0599",
            "No method or field found. Requires type or trait changes.",
        ),
        (
            r"error\[E0609\]",
            "field_not_found",
            "E0609",
            "Unknown field. Requires struct definition changes.",
        ),
        (
            r"error\[E0381\]",
            "uninitialized_variable",
            "E0381",
            "Use of possibly-uninitialized variable. Requires control-flow refactor.",
        ),
        (
            r"error\[E0499\]",
            "borrow_mismatch",
            "E0499",
            "Cannot borrow mutably more than once. Requires ownership refactor.",
        ),
        (
            r"error\[E0502\]",
            "borrow_mismatch",
            "E0502",
            "Cannot borrow immutably while mutable borrow active. Requires ownership refactor.",
        ),
        (
            r"error\[E0507\]",
            "move_error",
            "E0507",
            "Cannot move out of borrowed content. Requires clone/copy changes.",
        ),
    ]

    CARGO_PATTERNS: List[Tuple[str, str, str]] = [
        (
            r"error: failed to select a version for `?cc`?",
            "cargo_dependency_conflict",
            None,
            "Cargo dependency version conflict. Requires dependency or lockfile adjustment.",
        ),
        (
        ...
        (
            r"error: current package believes it.*workspace",
            "cargo_workspace_error",
            None,
            "Cargo workspace membership mismatch. Requires manifest correction.",
        ),
        (
            r"error: could not find `?Cargo\.toml`?",
            "cargo_manifest_path",
            None,
            "Cargo.toml not found in current directory. Add --manifest-path or create a workspace root Cargo.toml.",
        ),
        (
            r"error: manifest path",
            "cargo_manifest_path",
            None,
            "The requested Cargo.toml manifest path is invalid. Update the manifest or execution options.",
        ),
        (
            r"error: package collision",
            "cargo_workspace_error",
            None,
            "Cargo workspace package name collision. Requires manifest correction.",
        ),
        (
            r"error: failed to run custom build command",
            "cargo_build_script_error",
            None,
            "Build script or C-FFI link failure. Requires dependency or environment fix.",
        ),
        (
            r"ld: cannot find",
            "linker_error",
            None,
            "Linker cannot find symbol. Requires library or C-FFI dependency fix.",
        ),
        (
            r"undefined reference to",
            "linker_error",
            None,
            "Undefined C-FFI symbol. Requires library or binding fix.",
        ),
        (
            r"error: invalid array",
            "cargo_manifest_syntax",
            None,
            "Cargo.toml array syntax is invalid. Requires manifest rewrite.",
        ),
        (
            r"error: failed to parse manifest",
            "cargo_manifest_syntax",
            None,
            "Cargo.toml parse error. Requires manifest rewrite.",
        ),
    ]
    ...
    PYTHON_SYNTAX_PATTERNS: List[Tuple[str, str, str]] = [
        (
            ...
            "python_syntax",
            None,
            "Python syntax error; structural AST overlay can repair it.",
        ),
        (
            r"IndentationError:",
            "python_syntax",
            None,
            "Python indentation error; structural AST overlay can repair it.",
        ),
        (
            r"TabError:",
            "python_syntax",
            None,
            "Python tab/space error; structural AST overlay can repair it.",
        ),
    ]
    ...
    PYTHON_SEMANTIC_PATTERNS: List[Tuple[str, str, Optional[str], str]] = [
        (
            ...
            None,
            "Missing Python dependency. Requires installation or environment change.",
        ),
        (
            r"ImportError:",
            "python_import_error",
            None,
            "Python import error. Requires module or dependency fix.",
        ),
        (
            r"AttributeError:",
            "python_runtime_error",
            None,
            "Python attribute error. Requires logic or API fix.",
        ),
        (
            r"OSError:\s*.*cannot load library",
            "c_abi_library_load_error",
            None,
            "ctypes could not load the native shared library. Requires directory mapping or library build fix.",
        ),
        (
            r"FileNotFoundError:\s*.*Could not find native library",
            "c_abi_library_load_error",
            None,
            "Native shared library not found. Requires directory mapping or library build fix.",
        ),
        (
            r"undefined symbol:\s*\w+",
            "c_abi_symbol_error",
            None,
            "Native shared library is missing a required C-ABI symbol export. Requires symbol export fix.",
        ),
        (
        ...
        (
            ...
            "signature_mismatch",
            None,
            "Function called with the wrong number of positional arguments. The target signature should accept *args, **kwargs or the caller should be adjusted.",
        ),
        (
            r"TypeError:\s*(?:\w+\.)*\w+\(\)\s*missing\s+\d+\s+required\s+(?:positional\s+)?argument",
            "signature_mismatch",
            None,
            "Function called with missing required positional arguments. The target signature should accept *args, **kwargs or the caller should be adjusted.",
        ),
        (
            r"TypeError:",
            "python_runtime_error",
            None,
            "Python type error. Requires logic or signature fix.",
        ),
        (
            r"ValueError:",
            "python_runtime_error",
            None,
            "Python value error. Requires logic fix.",
        ),
        (
            r"KeyError:",
            "python_runtime_error",
            None,
            "Python key error. Requires logic fix.",
        ),
    ]
                ...
                result.update(
                    {
                        ...
                        "reason": reason
                        or "Structural Python syntax issue; an AST overlay can repair it.",
                        "summary": summary,
                    }
                )
                if frame:
                    ...
                    result["target_file"] = (
                        self._extract_target_from_command(command) or "main.py"
                    )
                return result
                ...
                result.update(
                    {
                        ...
                        "reason": reason,
                        "summary": reason
                        or f"Python runtime/semantic error ({error_type}).",
                    }
                )
                if frame:
                    result["target_file"] = frame["file"]
            ...
            for pattern, error_type, code, reason in self.RUST_SEMANTIC_PATTERNS:
                if re.search(pattern, log_text):
                    ...
                    if location:
                        ...
                        result["target_file"] = self._extract_target_from_command(
                            command
                        )
                    return result
